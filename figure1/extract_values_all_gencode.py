...
# fasta file input
all_gencode_file = Path(PurePath(base_path, f"gencode.v{release_num}.pc_transcripts.nopary.cdsonly.fa"))
...
motif_underrep_stds = [] 
mut_suscept_stds = []
for cnt, seq_record in enumerate(SeqIO.parse(all_gencode_file, "fasta")):    
...
df.to_csv(Path(PurePath(base_path, f"cdur.gencode.v{release_num}.pc_transcripts.nopary.cdsonly.csv")), index=False)
