...
for cnt, seq_record in enumerate(SeqIO.parse(refseq_file, "fasta")):    
    ...
    run1_file = Path(PurePath(base_path, f"cdur_mouse_refseq_run1", filename))
    run2_file = Path(PurePath(base_path, f"cdur_mouse_refseq_run2", filename))
    run3_file = Path(PurePath(base_path, f"cdur_mouse_refseq_run3", filename))
...
df.to_csv(Path(PurePath(base_path, f"cdur.gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.refseq.csv")), index=False)
