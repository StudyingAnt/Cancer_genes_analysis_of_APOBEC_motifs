...
for cnt, seq_record in enumerate(SeqIO.parse(refseq_file, "fasta")):    
    ...
    if run1_file.is_file() and run2_file.is_file() and run3_file.is_file():
        ...
        motif_underrep1 = rlt1.loc[rlt1["Property"] == "belowC_C_"]["Value"].values[0]
        motif_underrep2 = rlt2.loc[rlt2["Property"] == "belowC_C_"]["Value"].values[0]
        motif_underrep3 = rlt3.loc[rlt3["Property"] == "belowC_C_"]["Value"].values[0]

        mut_suscept1 = 1-rlt1.loc[rlt1["Property"] == "repTrFrac_belowC_C_"]["Value"].values[0]
        mut_suscept2 = 1-rlt2.loc[rlt2["Property"] == "repTrFrac_belowC_C_"]["Value"].values[0]
        mut_suscept3 = 1-rlt3.loc[rlt3["Property"] == "repTrFrac_belowC_C_"]["Value"].values[0]
...
df.to_csv(Path(PurePath(base_path, f"cdur.gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.refseq.cc.csv")), index=False)
