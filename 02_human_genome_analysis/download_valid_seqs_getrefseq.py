...
Entrez.api_key = "7e79f81ac06d3bf4ab32f38b0fe1de8cc409"
...
ncbi_hsapiens_file = Path(PurePath(base_path, "ncbi_human_clean.tsv"))

ncbi_hsapiens = pd.read_csv(ncbi_hsapiens_file, sep="\t")
gene_ids = ncbi_hsapiens.loc[ncbi_hsapiens["type"] == "PROTEIN_CODING"]["gene_id"].to_list()

genes_files_path = Path(PurePath(base_path, "homo_sapiens_genes"))
...
for batch_idx, start_idx in enumerate(starts):
    ...
    for i in range(len(record)):
        if "Gene-commentary_products" in record[i]["Entrezgene_locus"][0].keys():
            for product in record[i]["Entrezgene_locus"][0]["Gene-commentary_products"]:  
                ...
                if ref_id[0:2] == "NM" or ref_id[0:2] == "XM":
    ...
    while ref_is_done:
        ...
        if ref_ii+max_cnt >= ref_last_index:
            ref_ends.append(ref_last_index)
            break
        else:
            ref_ends.append(ref_ii+max_cnt)
            ref_ii = ref_ii+max_cnt
        ...
        for idx in range(len(subrecord)):
            ...
            for i in range(len(subrecord[idx]["GBSeq_feature-table"])):
                for key, value in subrecord[idx]["GBSeq_feature-table"][i].items():           
                    if key == "GBFeature_key" and value == "CDS":
                        ...
                        if "GBQualifier_value" in subrecord[idx]["GBSeq_feature-table"][i]["GBFeature_quals"][0].keys():
                            gene_symbol = subrecord[idx]["GBSeq_feature-table"][i]["GBFeature_quals"][0]["GBQualifier_value"]
                        else:               
                            gene_symbol = "NoSymbol"
            ...
            if froms and tos:
                ...
                if transcript_variant:
                    new_id = f"{ref_id}_Homo_sapiens_{gene_symbol}_{'_'.join(transcript_variant.split())}"
                else:
                    new_id = f"{ref_id}_Homo_sapiens_{gene_symbol}"
                ...
                if "N" in subrecord[idx]["GBSeq_sequence"][from_idx-1:to_idx].upper():
                    ...
                    SeqIO.write(seq_record, PurePath(base_path, "homo_sapiens_genes", f"{new_id}.fa"), "fasta")
...
output_file = PurePath(base_path, f"homo_sapiens_pc_transcripts.fa")
