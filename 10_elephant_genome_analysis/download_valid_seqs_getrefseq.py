...
ncbi_lafricana_file = Path(PurePath(base_path, "ncbi_lafricana_clean.tsv"))

ncbi_lafricana = pd.read_csv(ncbi_lafricana_file, sep="\t")
gene_ids = ncbi_lafricana.loc[ncbi_lafricana["type"] == "PROTEIN_CODING"]["gene_id"].to_list()
    ...
    for ref_batch_idx, ref_start_idx in enumerate(ref_starts):
        ...
        for idx in range(len(subrecord)):
            ...
            if transcript_variant:
                ...
                new_id = f"{ref_id}_Loxodonta_africana_{gene_symbol}_{'_'.join(transcript_variant.split())}"
            else:
                if "/" in gene_symbol:
                    gene_symbol = gene_symbol.replace("/", ".")

                new_id = f"{ref_id}_Loxodonta_africana_{gene_symbol}"
            ...
            if "N" in subrecord[idx]["GBSeq_sequence"][from_idx-1:to_idx].upper():
                ...
                SeqIO.write(seq_record, PurePath(base_path, "loxodonta_africana_genes", f"{new_id}.fa"), "fasta")
...
output_file = PurePath(base_path, f"loxodonta_africana_pc_transcripts.fa")
