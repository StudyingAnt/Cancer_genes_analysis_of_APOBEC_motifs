...
# fasta file input
refseq_file = Path(PurePath(base_path, f"pteropus_alecto_pc_transcripts.fa"))
...
for cnt, seq_record in enumerate(SeqIO.parse(refseq_file, "fasta")):    
    ...
    run1_file = Path(PurePath(base_path, f"pteropus_alecto", f"cdur_run1", filename))
    run2_file = Path(PurePath(base_path, f"pteropus_alecto", f"cdur_run2", filename))
    run3_file = Path(PurePath(base_path, f"pteropus_alecto", f"cdur_run3", filename))
    if run1_file.is_file() and run2_file.is_file() and run3_file.is_file():
        ...
        motif_underrep1 = rlt1.loc[rlt1["Property"] == "belowTY_C_"]["Value"].values[0]
        motif_underrep2 = rlt2.loc[rlt2["Property"] == "belowTY_C_"]["Value"].values[0]
        motif_underrep3 = rlt3.loc[rlt3["Property"] == "belowTY_C_"]["Value"].values[0]

        mut_suscept1 = 1-rlt1.loc[rlt1["Property"] == "repTrFrac_belowTY_C_"]["Value"].values[0]
        mut_suscept2 = 1-rlt2.loc[rlt2["Property"] == "repTrFrac_belowTY_C_"]["Value"].values[0]
        mut_suscept3 = 1-rlt3.loc[rlt3["Property"] == "repTrFrac_belowTY_C_"]["Value"].values[0]
...
df.to_csv(Path(PurePath(base_path, f"cdur.pteropus_alecto_pc_transcripts.tyc.csv")), index=False)
