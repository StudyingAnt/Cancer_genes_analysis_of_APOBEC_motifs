...
for cnt, seq_record in enumerate(SeqIO.parse(refseq_file, "fasta")):    
    ...
    run1_file = Path(PurePath(base_path, f"cdur_lafricana_run1", filename))
    run2_file = Path(PurePath(base_path, f"cdur_lafricana_run2", filename))
    run3_file = Path(PurePath(base_path, f"cdur_lafricana_run3", filename))
...
df.to_csv(Path(PurePath(base_path, f"cdur.loxodonta_africana_pc_transcripts.tyc.csv")), index=False)
