...
kegg_file <- paste(base_path, "mouse_enrichment_kegg.csv", sep = "")
...
enrichment_plot <- kegg_data %>%
  ...
  mutate(nlog10FDR=factor(nlog10FDR, levels=c(1,2,3,4,5,6))) %>%
...
out_file <- paste(base_path, "mouse_left-top_kegg_enrichment.png", sep="")
ggsave(out_file, plot = plot.fixed, dpi = 1200, width = 90, height = 100, units = "mm")

################################################################################
gobp_file <- paste(base_path, "mouse_enrichment_gobp.csv", sep = "")
...
enrichment_plot <- gobp_data %>%
  ...
  mutate(nlog10FDR=factor(nlog10FDR, levels=c(8,9,10,11,12,13,14,15))) %>%
  arrange(Fold.Enrichment) %>%    # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(Pathway=factor(Pathway, levels=Pathway)) %>%   # This trick update the factor levels
  ggplot( aes(x=Pathway, y=Fold.Enrichment, color=nlog10FDR)) +
  geom_segment( aes(xend=Pathway, yend=0), size = 0.8) +
  ...
  scale_colour_manual(
    name = "-log10(FDR)",
    values = mycolors[c(1,2,4,5,6,8,9,10)],
    drop = FALSE
...
out_file <- paste(base_path, "mouse_left-top_gobp_enrichment.png", sep="")
